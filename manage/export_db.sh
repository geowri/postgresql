...
EXPORT_DIR=$4  # Caminho principal onde os arquivos serão salvos
...
geom_tables=$(psql -d $DBNAME -U $USER -t -c "SELECT f_table_name FROM geometry_columns WHERE f_table_schema = '$SCHEMA';")
...
# Loop para exportar tabelas com geometria
for table in $geom_tables; do
  table=$(echo $table | xargs)  # Remove espaços em branco extras
  TABLE_DIR="$EXPORT_DIR/shapefiles/$table"  # Define o diretório específico para cada shapefile
  
  # Cria o diretório para a tabela, se não existir
  mkdir -p "$TABLE_DIR"
  
  echo "Exportando tabela com geometria $table para o diretório $TABLE_DIR..."
...
for table in $non_geom_tables; do
  table=$(echo $table | xargs)  # Remove espaços em branco extras
  TABLE_DIR="$EXPORT_DIR/csv/$table"  # Define o diretório específico para os arquivos CSV
